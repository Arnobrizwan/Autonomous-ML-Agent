...
class MissingValueDetector:
    ...
    def detect_missing(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        self.missing_info = {
            ...
            "columns_with_missing": data.isnull()
...
class OutlierDetector:
    ...
    def detect_outliers(
        ...
        logger.info(
            f"Detected {total_outliers} outliers across {len(numeric_columns)} numeric columns"
        )
    ...
    def get_outlier_columns(self) -> List[str]:
        ...
        return [
            col for col, info in self.outlier_info.items() if info["n_outliers"] > 0
        ]
