...
class PreprocessingPipeline:
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> 'PreprocessingPipeline':
        ...
        self.is_fitted = True
        logger.info(f"Preprocessing pipeline fitted with {len(self.feature_names_)} features")
    ...
    def _build_pipeline(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> None:
        ...
        if numeric_columns and self.config.handle_missing:
            ...
            transformers.append(('numeric_imputation', numeric_imputer, numeric_columns))
        ...
        if categorical_columns and self.config.handle_missing:
            ...
            transformers.append(('categorical_imputation', categorical_imputer, categorical_columns))
        
        # DateTime expansion
        if datetime_columns and self.config.datetime_expansion:
            datetime_expander = DateTimeExpander()
            transformers.append(('datetime_expansion', datetime_expander, datetime_columns))
        
        # Categorical encoding
        if categorical_columns:
            categorical_encoder = CategoricalEncoder(
                method=self.config.encode_categorical,
                max_categories=50
            )
            transformers.append(('categorical_encoding', categorical_encoder, categorical_columns))
...
def create_preprocessing_pipeline(config: Optional[PreprocessingConfig] = None) -> PreprocessingPipeline:
