...
from ..types import TaskType, TrialResult, DatasetProfile, ModelCard, LLMConfig
...
class ModelCardGenerator:
    ...
    def generate_card(self, 
                     ...
                     y: Optional[pd.Series] = None) -> ModelCard:
        """
        ...
        # Get best model
        best_result = max(trial_results, key=lambda x: x.score) if trial_results else None
        model_name = "Ensemble" if ensemble_model else (best_result.model_type.value if best_result else "Unknown")
        ...
        confusion_mat = None
        if task_type == TaskType.CLASSIFICATION and model and X is not None and y is not None:
            try:
    ...
    def _calculate_performance_metrics(self, 
        ...
        if trial_results:
            ...
            metrics.update({
                ...
                "cv_mean": np.mean(best_result.cv_scores) if best_result.cv_scores else 0,
                "cv_std": np.std(best_result.cv_scores) if best_result.cv_scores else 0,
    ...
    def _generate_limitations(self, 
        ...
        if dataset_profile:
            if dataset_profile.missing_ratio > 0.1:
                limitations.append(f"High missing data ratio ({dataset_profile.missing_ratio:.1%}) may affect performance")
            
            if dataset_profile.n_rows < 1000:
                limitations.append("Small dataset size may limit model generalization")
            
            if task_type == TaskType.CLASSIFICATION and dataset_profile.class_balance and dataset_profile.class_balance < 0.3:
                limitations.append("Class imbalance may affect model performance")
        
        limitations.append("Model performance may vary on data from different distributions")
    ...
    def _generate_recommendations(self, 
        ...
        if best_score < 0.7:
            recommendations.append("Consider collecting more data or feature engineering to improve performance")
        
        if performance_metrics.get("cv_std", 0) > 0.1:
            recommendations.append("High variance in CV scores suggests model instability - consider regularization")
        
        # Task-specific recommendations
        if task_type == TaskType.CLASSIFICATION:
            if performance_metrics.get("precision", 0) < performance_metrics.get("recall", 0):
                recommendations.append("Low precision suggests many false positives - consider threshold tuning")
            elif performance_metrics.get("recall", 0) < performance_metrics.get("precision", 0):
                recommendations.append("Low recall suggests many false negatives - consider class balancing")
    ...
    def _generate_llm_summary(self, 
        ...
        try:
            # Create prompt
            prompt = self._create_llm_prompt(trial_results, performance_metrics, dataset_profile, task_type)
...
def generate_model_card(trial_results: List[TrialResult],
                       ensemble_model: Optional[Any] = None,
                       task_type: TaskType = TaskType.CLASSIFICATION,
                       dataset_profile: Optional[DatasetProfile] = None,
                       model: Optional[Any] = None,
                       ...
                       llm_config: Optional[LLMConfig] = None) -> ModelCard:
    """Generate model card."""
