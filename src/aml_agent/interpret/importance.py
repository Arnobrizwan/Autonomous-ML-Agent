...
class FeatureImportanceAnalyzer:
    ...
    def _get_builtin_importance(self, model: BaseEstimator, X: pd.DataFrame) -> Dict[str, float]:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            logger.warning("Model has no built-in feature importance, falling back to permutation")
            return {}
    ...
    def analyze_feature_groups(self, 
                             importance_scores: Dict[str, float],
                             feature_groups: Optional[Dict[str, List[str]]] = None) -> Dict[str, float]:
        ...
        for group_name, features in feature_groups.items():
            group_score = sum(
                importance_scores.get(feature, 0)
    ...
    def _create_default_groups(self, feature_names: List[str]) -> Dict[str, List[str]]:
        ...
        for feature in feature_names:
            if any(pattern in feature.lower() for pattern in ["_year", "_month", "_day", "_hour", "_dow"]):
                groups["datetime"].append(feature)
            elif any(pattern in feature.lower() for pattern in ["_cat", "_cat_", "category"]):
    ...
    def compare_models_importance(self, 
    ...
    def get_importance_consensus(self, 
                               model_importance: Dict[str, Dict[str, float]]) -> Dict[str, float]:
...
def get_feature_importance(model: BaseEstimator,
                          X: pd.DataFrame,
                          y: pd.Series,
                          method: str = "auto") -> Dict[str, float]:
    """Get feature importance for a model."""
...
def get_top_features(importance_scores: Dict[str, float],
                    top_k: int = 10) -> List[Tuple[str, float]]:
    """Get top K most important features."""
    analyzer = FeatureImportanceAnalyzer()
...
def compare_models_importance(models: Dict[str, BaseEstimator],
                            X: pd.DataFrame,
                            y: pd.Series) -> Dict[str, Dict[str, float]]:
    """Compare feature importance across multiple models."""
    analyzer = FeatureImportanceAnalyzer()
