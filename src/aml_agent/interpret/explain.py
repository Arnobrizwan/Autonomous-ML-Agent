...
class ModelExplainer:
    ...
    def _explain_with_shap(
        self, model: Any, X: pd.DataFrame, y: pd.Series, task_type: TaskType
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(model, "predict_proba") and task_type == TaskType.CLASSIFICATION:
                ...
                if hasattr(model, "feature_importances_"):
                    ...
                    explainer = shap.KernelExplainer(
                        model.predict_proba,
                        X.sample(100, random_state=self.random_seed),
                    )
            else:
                # Use TreeExplainer for tree-based models
                if hasattr(model, "feature_importances_"):
                    ...
                    # Use KernelExplainer for other models
                    explainer = shap.KernelExplainer(
                        model.predict, X.sample(100, random_state=self.random_seed)
                    )
            ...
            # Get top features
            top_features = self.importance_analyzer.get_top_features(
                importance_dict, 10
            )
    ...
    def _explain_with_importance(
        ...
        feature_groups = self.importance_analyzer.analyze_feature_groups(
            importance_dict
        )
    ...
    def explain_prediction(
        self, model: Any, X: pd.DataFrame, instance_idx: int, task_type: TaskType
    ) -> Dict[str, Any]:
    ...
    def _plot_feature_importance(
        self, importance_dict: Dict[str, float], output_dir: Path
    ) -> str:
    ...
    def _plot_feature_groups(
        self, feature_groups: Dict[str, float], output_dir: Path
    ) -> str:
    ...
    def _plot_shap_summary(
        self, shap_values: np.ndarray, X: pd.DataFrame, output_dir: Path
    ) -> str:
        """Plot SHAP summary."""
...
def explain_model(
    model: Any, X: pd.DataFrame, y: pd.Series, task_type: TaskType, method: str = "auto"
) -> Dict[str, Any]:
...
def explain_prediction(
    model: Any, X: pd.DataFrame, instance_idx: int, task_type: TaskType
) -> Dict[str, Any]:
    """Explain a single prediction."""
    explainer = ModelExplainer()
...
def generate_explanation_report(
    model: Any, X: pd.DataFrame, y: pd.Series, task_type: TaskType, output_dir: Path
) -> Dict[str, str]:
