...
from ..types import (
    TaskType, MetricType, ModelType, RunMetadata, TrialResult, 
    LeaderboardEntry, BudgetClock, LLMConfig
)
from ..config import Config
from ..utils import (
    generate_run_id, profile_dataset, create_artifacts_dir, 
    save_metadata, detect_task_type, select_metric
)
from ..logging import get_logger
from ..preprocess import PreprocessingPipeline
from ..models import ModelTrainer, EnsembleBuilder
...
class AgentLoop:
    ...
    def _preprocess_data(self, X: pd.DataFrame, y: Optional[pd.Series]) -> Tuple[pd.DataFrame, pd.Series, TaskType]:
    ...
    def _initialize_components(self, task_type: TaskType):
        ...
        # Initialize trainer
        logger.info(f"Config metric: {self.config.metric}, type: {type(self.config.metric)}")
    ...
    def _create_optimization_plan(self, X: pd.DataFrame, y: pd.Series, task_type: TaskType) -> Any:
        ...
        logger.info(f"Created optimization plan: {len(plan.candidate_models)} models, "
    ...
    def _create_ensemble(self, X: pd.DataFrame, y: pd.Series, plan: Any):
        ...
        try:
            ...
            logger.info(f"Created {plan.ensemble_strategy.method} ensemble: "
    ...
    def _export_results(self, X: pd.DataFrame, y: pd.Series, task_type: TaskType):
        ...
        if self.ensemble_model:
            ...
            best_model = get_model_factory(best_result.model_type, task_type, best_result.params)
            best_model.fit(X, y)
...
def run_autonomous_ml(config: Config, 
                     X: pd.DataFrame, 
