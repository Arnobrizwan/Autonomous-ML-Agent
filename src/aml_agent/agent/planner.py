...
from ..types import (
    ModelType, TaskType, MetricType, DatasetProfile, 
    PlannerProposal, EnsembleConfig, LLMConfig
)
...
class LLMPlanner:
    ...
    def _allocate_budget_heuristic(self, models: List[ModelType], 
                                 total_budget: float) -> Dict[ModelType, int]:
    ...
    def _select_metric_heuristic(self, context: PlanningContext) -> MetricType:
        ...
        if context.task_type == TaskType.CLASSIFICATION:
            ...
            if context.dataset_profile.class_balance and context.dataset_profile.class_balance < 0.3:
    ...
    def _determine_ensemble_heuristic(self, context: PlanningContext) -> Optional[EnsembleConfig]:
...
"""
    ...
    def _parse_llm_response(self, response: str, context: PlanningContext) -> PlannerProposal:
    ...
    def update_plan(self, context: PlanningContext, 
                   current_results: List[Dict[str, Any]]) -> PlannerProposal:
...
def create_planning_context(dataset_profile: DatasetProfile,
                          task_type: TaskType,
                          available_models: List[ModelType],
                          time_budget_seconds: float,
                          historical_performance: Optional[Dict[str, float]] = None) -> PlanningContext:
