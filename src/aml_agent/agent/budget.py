...
class BudgetManager:
    ...
    def check_budget(self) -> bool:
        ...
        if self.budget_clock.is_expired() and not self.is_expired:
            ...
            logger.warning(f"Budget expired after {self.budget_clock.elapsed_seconds:.1f}s")
...
class BudgetAwareOptimizer:
    ...
    def optimize(self, objective_func: Callable, n_trials: int = 100) -> Any:
        ...
        logger.info(f"Completed {len(results)} trials in {time.time() - self.optimization_start:.1f}s")
        return results
...
def calculate_optimal_trials(time_budget_seconds: float, 
                           model_types: list,
                           n_samples: int,
                           n_features: int) -> dict:
    ...
    def monitor():
        while not manager.is_expired:
            ...
            if manager.check_budget():
                ...
                logger.info(f"Budget status: {progress:.1f}% used, "
