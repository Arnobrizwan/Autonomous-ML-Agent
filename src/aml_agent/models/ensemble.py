...
from ..types import TaskType, TrialResult, EnsembleConfig, ModelType
...
class EnsembleBuilder:
    ...
    def create_ensemble(self, 
                       ...
                       y: Optional[pd.Series] = None) -> BaseEstimator:
        """
    ...
    def _get_top_models(self, trial_results: List[TrialResult], top_k: int) -> List[TrialResult]:
        ...
        top_models = sorted(successful_results, key=lambda x: x.score, reverse=True)[:top_k]
        
        logger.info(f"Selected top {len(top_models)} models with scores: "
...
class WeightedVotingClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, trial_results: List[TrialResult], weights: np.ndarray, random_seed: int = 42):
...
def create_ensemble(trial_results: List[TrialResult],
                   task_type: TaskType,
                   top_k: int = 3,
                   method: str = "voting",
                   X: Optional[pd.DataFrame] = None,
...
def evaluate_ensemble_performance(ensemble: BaseEstimator,
                                 X: pd.DataFrame,
                                 y: pd.Series,
                                 task_type: TaskType,
                                 cv_folds: int = 5) -> Dict[str, float]:
...
def get_ensemble_weights(trial_results: List[TrialResult], top_k: int = 3) -> np.ndarray:
...
def compare_ensemble_methods(trial_results: List[TrialResult],
                            X: pd.DataFrame,
                            y: pd.Series,
                            task_type: TaskType,
                            top_k: int = 3) -> Dict[str, Dict[str, float]]:
