        ...
        for param, spec in space.items():
            if isinstance(spec, (list, tuple)):
                if (
                    ...
                    if param_type == "int":
                        optuna_space[param] = optuna.distributions.IntDistribution(
                            low, high
                        )
                    elif param_type == "log":
                        optuna_space[param] = optuna.distributions.FloatDistribution(
                            low, high, log=True
                        )
                    ...
                    optuna_space[param] = optuna.distributions.CategoricalDistribution(
                        spec
                    )
            else:
                # Single value
...
def get_search_space(
    model_type: ModelType, custom_space: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """Get search space for model type."""
