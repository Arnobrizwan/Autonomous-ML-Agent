...
from ..utils import create_sample_data, load_data, save_data
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.selectbox(
            "Choose a page",
            ["Dashboard", "Run Pipeline", "Model Leaderboard", "Model Cards", "Monitoring", "Settings"]
        )
                ...
                with st.expander(f"Run: {run_dir.name}"):
                    ...
                    if metadata_file.exists():
                        ...
                        with col1:
                            ...
                            st.write(f"**Best Score:** {metadata.get('best_score', 'N/A')}")
                        with col2:
                            st.write(f"**Best Model:** {metadata.get('best_model', 'N/A')}")
                            st.write(f"**Total Trials:** {metadata.get('total_trials', 'N/A')}")
        else:
    ...
    with st.form("pipeline_form"):
        ...
        with col1:
            data_source = st.selectbox(
                "Data Source",
                ["Upload File", "Use Sample Data", "Load from Path"]
            )
...
def show_leaderboard():
    ...
    if leaderboard_file.exists():
        ...
        if len(df) > 0:
            ...
            with col1:
                # Score by model type
                fig = px.bar(df, x='model_type', y='score', 
                           title='Score by Model Type')
                st.plotly_chart(fig, use_container_width=True)
            
            with col2:
                # Score vs Fit Time
                fig = px.scatter(df, x='fit_time', y='score', 
                               color='model_type',
...
def show_model_cards():
    ...
    card_selection = st.selectbox(
        "Select Model Card",
        [f"{name} - {card.name}" for name, card in model_cards]
    )
    
    selected_card = model_cards[st.selectbox(
        "Select Model Card",
        range(len(model_cards))
    )][1]
...
def show_monitoring():
    ...
    for metric_name, metric_data in metrics["metrics"].items():
        if metric_data["type"] == "counter":
            ...
            st.write(f"**{metric_name}**: {metric_data['mean']:.3f}s avg ({metric_data['count']} samples)")
